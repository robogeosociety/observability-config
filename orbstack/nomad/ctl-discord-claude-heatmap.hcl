# Nomad service job supervising the OrbStack `discord-claude-heatmap` container (the live
...
job "ctl-discord-claude-heatmap" {
  namespace   = "orbstack"
  type        = "service"
  datacenters = ["*"]
  ...
  group "supervise" {
    ...
    task "track" {
      ...
      config {
        command = "/bin/zsh"
        args    = ["${var.supervise}", "discord-claude-heatmap"]
      }

# Nomad service job supervising the OrbStack `grafana-renderer` container. One file per
...
# (or directly: nomad job run ctl-grafana-renderer.hcl). See supervise.sh for the lifecycle.
variable "supervise" {
  type    = string
  default = "/Volumes/dev/observability/orbstack/nomad/supervise.sh"
}

job "ctl-grafana-renderer" {
  type        = "service"
  datacenters = ["*"]
  ...
  group "supervise" {
    ...
    task "track" {
      ...
      config {
        command = "/bin/zsh"
        args    = ["${var.supervise}", "grafana-renderer"]
      }

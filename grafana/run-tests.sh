...
PY_DIRS=("$GRAFANA/tests" "$GRAFANA/dev-status" "$GRAFANA/claude-usage-collector" "$ROOT/discord-mini-mem" "$ROOT/discord-orbstack-mem" "$GRAFANA/anomaly-detector" "$ROOT/coordination" "$ROOT/influxdb/tests" "$ROOT/orbstack/tests")
